...
TemplateError.prototype.extract_code = function(data) {
    if(data.myself && data.myself.__self__ && data.myself.__self__.code) {
...
gtpl.deepCopy = function(data, escapeStringFunction, ignore_prefix, skip_prototype) {
    ignore_prefix = ignore_prefix || '__';
    var recursion_list = [];
    var recursion_key = '__rec' + get_uid();
    var ignore_prefix_length = ignore_prefix ? ignore_prefix.length : 0;
        ...
        if (typeof(data) == "string") {
            ...
            if (!skip_prototype) {
                if(!Object.setPrototypeOf) {
                    ...
                    o = Object.create(Object.getPrototypeOf(data));
                }
                else {
                    Object.setPrototypeOf(o, data);
                }
            }
            ...
            for (var j = 0; j < keys.length; j++) {
                var key = keys[j];
                if (key !== recursion_key && key.substring(0,ignore_prefix_length) !== ignore_prefix)
                    o[key] = arguments.callee(data[key]);
            }
