...
(function() {
	...
	TemplateNode.prototype.compile = function(d) {
		...
		if(config.introspection_mode == 'names') {
			...
			res += TAB(d) + format('{0}.__self__ = {name: "{1}", namespace: "{2}", source: "{3}", pos_start: {4}, pos_end: {5}, line_start: {6}, line_end: {7}};', 
				function_name, this.template_name.join('.'), this.namespace.join('.'), this.source, this.pos, this.last_pos, this.line, this.last_line) + LB;
		}
		else if(config.introspection_mode == 'code_debug') {
